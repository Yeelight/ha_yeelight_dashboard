...
- 提供 `custom:yeelight-dashboard-hero-card`、`status-card`、`notice-card`、`light-card`、`rooms-card`、`room-card`、`devices-card`、`routines-card`、`environment-card`、`ecosystem-card`、`health-card` 等产品型复合卡片；这些卡片通过 `window.customCards` 注册 HA 可视化编辑器，支持接管后手工编辑，但定位仍是 Dashboard 产品卡，不替代轻量 `ha_yeelight_cards` 卡片包。
